    ...
    try {
      ...
      if (state is RatingInitial) {
        ...
        var ratingData = await ratingRepository.getRating(
            event.gameMode,
            event.ratingType,
            currentState.ratingData.ratings.length,
            recordsPerRequest);
        yield RatingDataLoaded(
            ratingData: LeaderboardModel(
                currentState.ratingData.ratings + ratingData.ratings,
                ratingData.currentPlace,
                ratingData.calibrationGamesLeft),
            hasReachedMax: ratingData.ratings.isEmpty);
      }
