...
class AuthRepositoryException extends ApplicationException {
  AuthRepositoryException([String message, Exception innerException])
...
class AuthRepository {
  ...
  Future<void> signUpWithGoogle() async {
    try {
      ...
      if (googleUser == null) {
        throw AuthRepositoryException("Google Sign In failed.");
      }
      var signInMethods =
  ...
  Future<void> signInWithGoogle() async {
    try {
      ...
      if (signInMethods.contains(GoogleAuthProvider.PROVIDER_ID)) {
        ...
        throw AuthRepositoryException(
            "User with this Google account is not registered.");
      }
    } on Exception {
      await _googleSignIn.signOut();
      rethrow;
